...
**Connecting** — the connection string comes from `DATABASE_URL`, falling back to `api/.env` (see `api/.env.example`, which explains why the URL points at the Supavisor pooler and carries `default_query_exec_mode=exec`). `go run ./cmd/pgcheck` prints the server version and table counts to verify it. Because the pooler runs in transaction mode, nothing in this package may create a named prepared statement — that is why `Insert` and `ApplyOwnerRules` build statements inline rather than calling `Prepare`.
...
- `owner` is the partner whose personal budget the transaction falls in; null means joint. API endpoints take `?owner=` to filter, where the reserved value `joint` selects the null rows
