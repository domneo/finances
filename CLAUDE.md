...
**Demo dataset** — the database holds two households, not one. `transactions`, `owner_rules` and `budget_contributors` each carry an `is_dummy` flag, so a made-up household (Chiikawa and Hachiware) sits beside the real one under the same schema, the same queries and the same code paths — which is what lets a public deployment show the app working without anybody's bank statements in it. A second database would drift out of shape the moment a column was added to only one of them.

`DEMO_MODE` picks which one a process serves, read once by `txns.Open` from the environment or `api/.env`. Unset or false — the default, and what a private deployment wants — serves the real household; true serves only the dummy rows. Anything `strconv.ParseBool` rejects counts as false, so a typo fails towards real data staying hidden rather than towards showing it, and the server logs a line at startup when the flag is on.

The flag is not a filter a query may forget. `txns.Dataset(qualifier)` returns the predicate (`is_dummy IS TRUE` / `IS NOT TRUE`, taking a table alias like `"t."` where the query uses one) and every read of those three tables names it — `whereClause` in `main.go` leads every transactions query with it, and `loadOwnerRules`, `loadBudgetContributors`, `Owners`, `ApplyOwnerRules` and `SetOwner` each add it themselves. It is `IS NOT TRUE` rather than `= false` because the column is nullable, and a row written before the flag existed or by hand in the Supabase editor holds `NULL` and belongs to the real household. `Insert` writes `is_dummy` too, stamping each row with the dataset that imported it; nothing in the API moves a row between the two, which is a deliberate edit in Supabase. Deletes and single-row reads are scoped as well, so a demo deployment cannot reach a real transaction by guessing its id. `categories` and `category_keywords` have no flag — a category name and a keyword are not personal data, and both households share them.

The demo household's own rows are generated, not typed: `DEMO_MODE=true go run ./cmd/dummyseed` clears the dummy dataset and rebuilds it from a fixed random seed — twelve months back from today, which is the window the dashboard averages a recurring bill over, so every panel lands populated. It refuses to run unless `DEMO_MODE` is on, since that flag is the only thing keeping a thousand invented rows out of the real household, and the clear-out it starts with goes through `txns.Dataset` for the same reason. Owner rules go in before the transactions so `AssignOwners` can tag the personal ones by matching references, exactly as an import does, rather than the generator asserting owners it already knew; two rows are assigned by hand instead, so `owner_source` has both kinds in it. Re-running replaces the household rather than doubling it, and `-n` prints what it would write. `budget_contributors` for the two demo partners is not seeded — it is the one table that has to exist first, since `owner` is a foreign key to it.
...
**Design system** — the styling comes from `bleed`, a separate repo, whose built CSS, fonts, icon sprite and runtime script have to sit at `ui/src/lib/assets/ds` for `+layout.svelte` to import them (as `?url`/`?raw` imports, wired into `<svelte:head>`). That directory is generated, not committed: `ui/scripts/sync-ds.js` runs ahead of `dev`, `build` and `check` and copies `dist` out of the `bleed` dependency (a git dependency pinned in `pnpm-lock.yaml`), unless the directory is already a working symlink, which it leaves alone. So a deploy gets the pinned commit, and a checkout next to a local `bleed` clone can keep `ln -s ../../../../../bleed/dist ui/src/lib/assets/ds` and see design changes live without reinstalling. It was previously that symlink and nothing else, committed, which built here and failed anywhere the sibling repo did not exist. Run `pnpm --filter ui sync:bleed` to refresh the copy by hand, and `pnpm update bleed` in `ui/` to move to a newer design system.
...
- The `transactions` table columns: `id`, `date`, `account`, `category`, `reference`, `amount`, `currency`, `owner`, `owner_source`, `is_dummy`
- `is_dummy` marks the row as belonging to the demo household rather than the real one; `NULL` counts as real. It is on `owner_rules` and `budget_contributors` too, and every read filters on it — see **Demo dataset**
